...
class Cab:  # or call it 'Driver' , logic handling is same
...
import math
def distance_computer(A,B):    # pickup & drop is 1-D plane for now, can be extended to lat/long easily
...
class DistanceBasedFareAlgo(FareGeneratorAlgo):
    def calculate_fare(self, tarrif_rate:float, tarrif_atomic_unit:int, ride:Ride)-> float:
        distance_traversed =  distance_computer(ride.drop_location,ride.pickup_location)
...
class FareGeneratorService:
    # implemented for time-based algo for now
    ...
    # can implement tarrif_xyz_algo for all the algos later (dist, etc..)
...
ride1 = Ride(cab1, p1, 1, 10,pickup_time,drop_time )    # pickup & drop is 1-D plane for now, can be extended to lat/long easily
